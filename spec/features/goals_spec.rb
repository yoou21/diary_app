...
RSpec.feature "Goal Management", type: :feature do
  ...
  scenario '目標の詳細ページにアクセスし、日記を追加するリンクが表示されること' do
    ...
    expect(page).to have_link('日記を追加', href: new_diary_path(goal_id: goal.id))  # リンクが正しく表示されること
    click_link '日記を追加'  # リンクをクリック
    expect(current_path).to eq(new_diary_path(goal_id: goal.id))  # 日記作成ページに遷移すること
  end

  scenario '目標を作成することができる' do
    visit new_goal_path  # 新しい目標作成ページに遷移
    fill_in 'タイトル', with: '新しい目標'  # タイトルを入力
    fill_in '締め切り', with: (Date.today + 7.days).to_s  # 締め切りを入力
    select '未達成', from: 'ステータス'  # ステータスを選択
    click_button '目標を作成'  # 目標を作成
