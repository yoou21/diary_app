  ...
  def up
    User.find_each do |user|
      ...
      unless user.emotions.exists?(date: Date.today, name: '嬉しい')
        user.emotions.create(name: '嬉しい', word: '喜び', score: 3, date: Date.today, goal: goal)
      end
      unless user.emotions.exists?(date: Date.today, name: '悲しい')
        user.emotions.create(name: '悲しい', word: '悲しみ', score: -1, date: Date.today, goal: goal)
      end
      unless user.emotions.exists?(date: Date.today, name: '楽しい')
        user.emotions.create(name: '楽しい', word: '楽しみ', score: 2, date: Date.today, goal: goal)
      end
      unless user.emotions.exists?(date: Date.today, name: '怒り')
        user.emotions.create(name: '怒り', word: '怒り', score: -2, date: Date.today, goal: goal)
      end
    end
  end
